...
export class DatabaseConnectionService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (process.env.NODE_ENV === 'test') {
      this.logger.log('Database connection service initialized (test mode - health checks disabled)');
      return;
    }
  ...
  async getPoolStats() {
    ...
    return {
      ...
      utilizationRate: status.totalCount > 0 ? (status.activeConnections / status.totalCount) * 100 : 0,
