...
describe('DatabasePerformanceController', () => {
  ...
  beforeEach(async () => {
    mockService = {
      ...
      getConfigurationRecommendations: jest.fn().mockReturnValue(['Config recommendation']),
      getIndexRecommendations: jest.fn().mockReturnValue(['Index recommendation']),
  ...
  describe('analyzeQuery', () => {
    ...
    it('should handle query analysis without params', async () => {
      ...
      expect(mockService.analyzeQuery).toHaveBeenCalledWith('SELECT * FROM test', undefined);
    });
  });
});