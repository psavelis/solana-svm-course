...
describe('DatabasePerformanceService', () => {
  ...
  describe('getPerformanceReport', () => {
    it('should return a comprehensive performance report', async () => {
      // Mock the database queries
      mockDbConnection.executeQuery = jest.fn()
    ...
    it('should handle errors gracefully', async () => {
      mockDbConnection.executeQuery = jest.fn().mockRejectedValue(new Error('DB Error'));
  ...
  describe('getIndexUsageStats', () => {
    it('should return index usage statistics', async () => {
      ...
      mockDbConnection.executeQuery = jest.fn().mockResolvedValue(mockIndexData);
    ...
    it('should handle query errors', async () => {
      mockDbConnection.executeQuery = jest.fn().mockRejectedValue(new Error('Query failed'));
  ...
  describe('getTableStats', () => {
    it('should return table statistics', async () => {
      ...
      mockDbConnection.executeQuery = jest.fn().mockResolvedValue(mockTableData);
  ...
  describe('generateRecommendations', () => {
    it('should generate recommendations for unused indexes', () => {
      const report = {
        indexStats: [
          { indexName: 'unused_idx', tableName: 'test', columnNames: [], indexSize: 1000, usageCount: 0, lastUsed: null, isUsed: false },
        ],
        slowQueries: [],
        tableStats: [],
        recommendations: [],
      ...
      expect(recommendations.some(rec => rec.includes('unused indexes'))).toBe(true);
    });
    ...
    it('should generate recommendations for large tables', () => {
      const report = {
        ...
        slowQueries: [],
        tableStats: [
          { tableName: 'large_table', rowCount: 1000000, tableSize: 200 * 1024 * 1024, indexSize: 50 * 1024 * 1024 }, // 200MB
        ],
        recommendations: [],
        generatedAt: new Date(),
      };
      ...
      expect(recommendations.some(rec => rec.includes('partitioning'))).toBe(true);
    });
  ...
  describe('getConfigurationRecommendations', () => {
    it('should return configuration recommendations', () => {
      ...
      expect(recommendations.length).toBeGreaterThan(0);
      expect(recommendations.some(rec => rec.includes('work_mem'))).toBe(true);
    });
  });
  ...
  describe('getIndexRecommendations', () => {
    it('should return index recommendations', () => {
      ...
      expect(Array.isArray(recommendations)).toBe(true);
      expect(recommendations.length).toBeGreaterThan(0);
      expect(recommendations.some(rec => rec.includes('composite indexes'))).toBe(true);
    });
  });
});