...
describe('FeeOptimizationService', () => {
  ...
  beforeEach(async () => {
    ...
    mockFeeService = {
      ...
      getFeeRecommendations: jest.fn().mockResolvedValue({
        min: { baseFee: 5000, priorityFee: 0, totalFee: 5000, computeUnits: 5000, feePayer: '' },
        low: { baseFee: 5000, priorityFee: 1000, totalFee: 6000, computeUnits: 5000, feePayer: '' },
        medium: { baseFee: 5000, priorityFee: 2000, totalFee: 7000, computeUnits: 5000, feePayer: '' },
        high: { baseFee: 5000, priorityFee: 5000, totalFee: 10000, computeUnits: 5000, feePayer: '' },
        veryHigh: { baseFee: 5000, priorityFee: 10000, totalFee: 15000, computeUnits: 5000, feePayer: '' },
        unsafeMax: { baseFee: 5000, priorityFee: 50000, totalFee: 55000, computeUnits: 5000, feePayer: '' },
      }),
    };
  ...
  describe('Network analysis', () => {
    ...
    it('should handle network analysis errors gracefully', async () => {
      ...
      mockFeeService.getFeeMarketStats.mockRejectedValueOnce(new Error('Fee service error'));
  ...
  describe('Strategy selection', () => {
    it('should select optimal strategy based on scoring', () => {
      ...
      const selected = (service as any).selectOptimalStrategy(mockResults, preferences);
