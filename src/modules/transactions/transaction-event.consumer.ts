...
export class TransactionEventConsumer implements OnModuleInit {
  ...
  async handleTransactionEvent(
    @Payload() message: { key: string; value: TransactionEvent; headers?: Record<string, string> },
    ...
    try {
      this.logger.log(`Received transaction event: ${event.eventType} for transaction ${event.transactionId} (Retry: ${retryCount})`);
      ...
      this.logger.error(`Failed to process transaction event ${event.eventType}: ${event.transactionId}`, error);

      if (retryCount < MAX_RETRIES) {
         this.logger.log(`Retrying transaction event ${event.transactionId} (Attempt ${retryCount + 1}/${MAX_RETRIES})`);
         await this.retryMessage(message, retryCount + 1);
      } else {
         this.logger.error(`Max retries reached for transaction ${event.transactionId}. Sending to DLQ.`);
  ...
  private async handleTransactionStatusUpdated(event: TransactionEvent): Promise<void> {
    this.logger.log(`Processing transaction status update: ${event.transactionId} (${event.metadata?.previousStatus} -> ${event.status})`);
  ...
  private async handleTransactionFailed(event: TransactionEvent): Promise<void> {
    this.logger.error(`Processing transaction failed: ${event.transactionId}`, event.metadata?.error);
  ...
  private async retryMessage(
    originalMessage: { key: string; value: TransactionEvent; headers?: Record<string, string> },
    retryCount: number
  ): Promise<void> {
      const headers = {
      ...
      await this.kafkaClient.emit('transactions', {
  ...
  private async sendToDeadLetterQueue(
    ...
    try {
      ...
      this.logger.warn(`Sent message to dead letter queue: ${originalMessage.value.transactionId}`, dlqMessage.error);
    } catch (dlqError) {
