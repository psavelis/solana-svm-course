...
describe('TransactionEventConsumer', () => {
  ...
  describe('onModuleInit', () => {
    it('should subscribe to transactions and dlq topics and connect', async () => {
      ...
      expect(kafkaClientMock.subscribeToResponseOf).toHaveBeenCalledWith('transactions');
      expect(kafkaClientMock.subscribeToResponseOf).toHaveBeenCalledWith('transaction-events-dlq');
  ...
  describe('handleTransactionEvent', () => {
    it('should handle transaction created event', async () => {
      const createdEvent = { ...mockEvent, eventType: TransactionEventType.CREATED };
    ...
    it('should handle transaction confirmed event', async () => {
      const confirmedEvent = { ...mockEvent, eventType: TransactionEventType.CONFIRMED };
    ...
    it('should retry processing errors if retry count < MAX', async () => {
      ...
      const errorEvent = { ...mockEvent, eventType: TransactionEventType.CREATED };
      ...
      consumer['handleTransactionCreated'] = jest.fn().mockRejectedValue(new Error('Processing failed'));
    ...
    it('should send to DLQ if retry count >= MAX', async () => {
      ...
      const errorEvent = { ...mockEvent, eventType: TransactionEventType.CREATED };
      const errorMessage = { ...mockMessage, value: errorEvent, headers: { 'retry-count': '3' } };
      ...
      expect(kafkaClientMock.emit).toHaveBeenCalledWith('transaction-events-dlq', expect.any(Object));
