...
export class SvmService {
  ...
  async executeProgram(
    ...
    try {
      ...
      await this.checkGasMeter(
        dto.programId,
        dto.maxComputeUnits || program.maxComputeUnits,
      );
      ...
      savedExecution.computeUnitsUsed = Math.min(
        savedExecution.computeUnitsAllocated,
        100000,
      ); // Simplified
  ...
  async executeParallel(
    ...
    if (
      dto.maxTotalComputeUnits &&
      totalComputeUnits > dto.maxTotalComputeUnits
    ) {
      throw new BadRequestException(
        "Total compute units exceed maximum allowed",
      );
    }
  ...
  async getExecutionMetrics(dto: ExecutionMetricsDto): Promise<any> {
    const startDate = new Date(
      Date.now() - dto.timeRangeHours * 60 * 60 * 1000,
    );
  ...
  async queryGasMeters(
    ...
    if (query.usageThresholdPercent) {
      ...
      qb.andWhere(
        "(meter.gasUsed * 100 / meter.gasAllocated) >= :usagePercent",
        { usagePercent },
      );
    }
  ...
  async consumeGas(
    ...
    meter.efficiencyRating = Math.max(
      0,
      100 - (meter.gasUsed * 100) / meter.gasAllocated,
    );

    // Check threshold
    const usagePercent = (meter.gasUsed * 100) / meter.gasAllocated;
    if (
      usagePercent >= meter.alertThresholdPercent &&
  ...
  async getProgramStats(programId: string): Promise<any> {
    ...
    const totalComputeUnits = executions.reduce(
      (sum, e) => sum + (e.computeUnitsUsed || 0),
      0,
    );
    const totalGasCost = executions.reduce(
      (sum, e) => sum + (e.gasCost || 0),
      0,
    );
