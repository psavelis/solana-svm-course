...
import { Connection, PublicKey, Transaction, SystemProgram, sendAndConfirmTransaction, ComputeBudgetProgram } from '@solana/web3.js';
...
export class SvmService {
  ...
  async queryPrograms(query: ProgramQueryDto): Promise<{ programs: Program[]; total: number }> {
    ...
    if (query.programType) {
      qb.andWhere('program.programType = :programType', { programType: query.programType });
    }
  ...
  async deployProgram(id: string, dto: DeployProgramDto, signer: any): Promise<Program> {
    ...
    try {
      ...
      // Send transaction
      const signature = await sendAndConfirmTransaction(this.connection, transaction, [signer, programKeypair]);
  ...
  async executeProgram(dto: ExecuteProgramDto, signer: any): Promise<RuntimeExecution> {
    ...
    let savedExecution = await this.executionRepository.save(execution);
  ...
  async queryExecutions(query: RuntimeExecutionQueryDto): Promise<{ executions: RuntimeExecution[]; total: number }> {
    const qb = this.executionRepository.createQueryBuilder('execution')
      .leftJoinAndSelect('execution.program', 'program');

    if (query.programId) {
      qb.andWhere('execution.programId = :programId', { programId: query.programId });
    }

    if (query.transactionId) {
      qb.andWhere('execution.transactionId = :transactionId', { transactionId: query.transactionId });
    }
    ...
    if (query.executionType) {
      qb.andWhere('execution.executionType = :executionType', { executionType: query.executionType });
    }

    if (query.startDate) {
      qb.andWhere('execution.createdAt >= :startDate', { startDate: new Date(query.startDate) });
    }

    if (query.endDate) {
      qb.andWhere('execution.createdAt <= :endDate', { endDate: new Date(query.endDate) });
    }

    if (query.minComputeUnits) {
      qb.andWhere('execution.computeUnitsUsed >= :minComputeUnits', { minComputeUnits: query.minComputeUnits });
    }

    if (query.maxComputeUnits) {
      qb.andWhere('execution.computeUnitsUsed <= :maxComputeUnits', { maxComputeUnits: query.maxComputeUnits });
    }
  ...
  async queryGasMeters(query: GasMeterQueryDto): Promise<{ meters: GasMeter[]; total: number }> {
    ...
    if (query.programId) {
      qb.andWhere('meter.programId = :programId', { programId: query.programId });
    }

    if (query.accountId) {
      qb.andWhere('meter.accountId = :accountId', { accountId: query.accountId });
    }

    if (query.meterType) {
      qb.andWhere('meter.meterType = :meterType', { meterType: query.meterType });
    }
  ...
  async consumeGas(identifier: string, gasAmount: number, operation?: string): Promise<void> {
    ...
    if (usagePercent >= meter.alertThresholdPercent && meter.status === GasMeterStatus.ACTIVE) {
      this.logger.warn(`Gas meter ${meter.id} exceeded ${meter.alertThresholdPercent}% usage threshold`);
    }
  ...
  async checkGasMeter(identifier: string, requiredGas: number): Promise<void> {
    ...
    if (requiredGas > meter.gasLimitPerOperation) {
      throw new BadRequestException(`Gas required (${requiredGas}) exceeds limit per operation (${meter.gasLimitPerOperation})`);
    }
  }
  ...
  async getProgramStats(programId: string): Promise<any> {
    ...
    const avgExecutionTime = executions.reduce((sum, e) => sum + (e.executionTimeMs || 0), 0) / totalExecutions;
