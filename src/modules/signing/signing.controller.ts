...
export class SigningController {
  ...
  async getHardwareWalletPublicKey(@Body() config: HardwareWalletConfig): Promise<string> {
  ...
  async signTransactionWithHardwareWallet(
    ...
    return await this.signingService.signTransactionWithHardwareWallet(transaction, body.config);
  }
  ...
  async signMessageWithHardwareWallet(
    ...
    return await this.signingService.signMessageWithHardwareWallet(messageBytes, body.config);
  }
  ...
  async createMultiSigTransaction(@Body() dto: CreateMultiSigTransactionDto): Promise<string> {
    ...
    return await this.signingService.createMultiSigTransaction(dto.multiSigAddress, transaction);
  }
  ...
  async signMultiSigTransaction(
    @Body() dto: SignMultiSigTransactionDto,
  ): Promise<MultiSigTransaction> {
    return await this.signingService.signMultiSigTransaction(dto.txId, dto.signerPrivateKey);
  }
  ...
  async createOfflineSigningRequest(
    @Body() dto: CreateOfflineSigningRequestDto,
  ): Promise<OfflineSigningRequest> {
    const transaction = new (await import('@solana/web3.js')).Transaction();
    Object.assign(transaction, dto.transactionData);
  ...
  createOfflineMessageSigningRequest(
    @Body() dto: CreateOfflineMessageSigningRequestDto,
  ): OfflineSigningRequest {
    const messageBytes = Buffer.from(dto.message, 'base64');
