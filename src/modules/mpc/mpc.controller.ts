...
import { MpcService, CreateMpcWalletRequest, MpcWalletResponse, KeyShareResponse, SignTransactionRequest, SignatureReconstructionResult } from './mpc.service';
...
export class MpcController {
  ...
  @Post('wallets')
  @ApiOperation({ summary: 'Create a new MPC wallet with distributed key generation' })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({ summary: 'Get key shares for a wallet (requires participant authentication)' })
  @ApiResponse({
    status: 200,
  ...
  @Post('sign')
  @ApiOperation({ summary: 'Sign a transaction using MPC threshold signatures' })
  @ApiResponse({
    status: 201,
    description: 'Transaction signed successfully using MPC',
    type: Object,
  })
  async signTransaction(@Body() request: SignTransactionDto): Promise<SignatureReconstructionResult> {
