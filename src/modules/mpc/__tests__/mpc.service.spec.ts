...
describe("MpcService", () => {
  ...
  beforeEach(async () => {
    ...
    mpcWalletRepository = module.get<Repository<MpcWallet>>(
      getRepositoryToken(MpcWallet),
    );
    keyShareRepository = module.get<Repository<KeyShare>>(
      getRepositoryToken(KeyShare),
    );
  });
  ...
  describe("revokeKeyShare", () => {
    ...
    it("should throw error for non-existent share", async () => {
      ...
      await expect(
        service.revokeKeyShare("mpc_123", "alice", 0),
      ).rejects.toThrow("Key share not found for participant alice");
    });
  });
});
