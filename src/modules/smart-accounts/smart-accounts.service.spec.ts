...
describe('SmartAccountsService', () => {
  ...
  describe('createSmartAccount', () => {
    it('should create and cache account', async () => {
      const result = await service.createSmartAccount('owner123', mockSmartAccount.rules);
      ...
      expect(kafkaMock.emit).toHaveBeenCalledWith('smart-account.created', expect.any(Object));
  ...
  describe('validateTransaction', () => {
    it('should validate successfully using cached rules', async () => {
      redisMock.get.mockImplementation((key) => {
        if (key.includes(':rules')) return Promise.resolve(JSON.stringify(mockSmartAccount.rules));
        if (key.includes(':spent')) return Promise.resolve('500');
        return null;
      });

      const result = await service.validateTransaction('smart-addr', 100, 'prog1');
      expect(result.valid).toBe(true);
    });

    it('should fail if daily limit exceeded', async () => {
      redisMock.get.mockImplementation((key) => {
        if (key.includes(':rules')) return Promise.resolve(JSON.stringify(mockSmartAccount.rules)); // Limit 1000
    ...
    it('should fail if program not allowed', async () => {
       redisMock.get.mockResolvedValueOnce(JSON.stringify(mockSmartAccount.rules));
       
       const result = await service.validateTransaction('smart-addr', 10, 'evil-prog');
       expect(result.valid).toBe(false);
    ...
    it('should fetch from DB if cache miss', async () => {
      ...
      const result = await service.validateTransaction('smart-addr', 10, 'prog1');
      
      expect(repositoryMock.findOne).toHaveBeenCalledWith({ where: { smartAccountAddress: 'smart-addr' } });
    ...
    it('should fail if account is not active', async () => {
        redisMock.get.mockResolvedValueOnce(null);
        repositoryMock.findOne.mockResolvedValue({ ...mockSmartAccount, status: SmartAccountStatus.DISABLED });
  ...
  describe('recordTransaction', () => {
    it('should increment spend in redis', async () => {
      ...
      expect(redisMock.incrby).toHaveBeenCalledWith(expect.stringContaining(':spent:'), 500);
