...
export class SmartAccountsService {
  ...
  async validateTransaction(smartAccountAddress: string, amount: number, programId: string): Promise<{ valid: boolean; reason?: string }> {
    // Try to get rules from Redis first
    let rulesStr = await this.redisClient.get(`smart-account:${smartAccountAddress}:rules`);
    let rules;

    if (!rulesStr) {
      const account = await this.smartAccountRepository.findOne({ where: { smartAccountAddress } });
      if (!account) return { valid: false, reason: 'Account not found' };
      if (account.status !== SmartAccountStatus.ACTIVE) return { valid: false, reason: 'Account not active' };
    ...
    if (rules.maxDailySpend) {
      ...
      if (newTotal > rules.maxDailySpend) {
        return { valid: false, reason: `Daily spend limit exceeded. Limit: ${rules.maxDailySpend}, Current: ${currentSpent}, Attempted: ${amount}` };
      }
    }
  ...
  async findByAddress(smartAccountAddress: string): Promise<SmartAccount> {
    return this.smartAccountRepository.findOne({ where: { smartAccountAddress } });
  }
}
